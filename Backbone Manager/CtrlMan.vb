...
Class CtrlMan 'Control Manager
    ...
    Public Class Fill
        ...
        Shared Sub GetCities(ByRef CityList As ComboBox, ByRef bs As BindingSource, State As Integer)
            ...
            bs.DataSource = DbMan.ReadDB("SELECT * FROM Localidades WHERE state_id=" & State & " ORDER BY nombre",
                                         My.Settings.currentcon)
        ...
        Shared Sub GetStates(ByRef StateList As ComboBox, ByRef bs As BindingSource)
            Dim sql(5) As String
            If My.Settings.currentcon <> "" Then
                StateList.BeginUpdate()
                bs.DataSource = DbMan.ReadDB("SELECT * FROM provincias ORDER BY nombre", My.Settings.currentcon)
